...
export function OpmeSelection({ selectedItems, setSelectedItems, procedure }: OpmeSelectionProps) {
  ...
  return (
    <Card className="bg-white rounded-lg shadow-md p-6 mb-6">
      <h3 className="text-lg font-medium text-neutral-800 mb-4">Seleção de OPME</h3>
      
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <Label htmlFor="search-opme">Pesquisar OPME</Label>
          <div className="flex items-center text-primary-500 text-sm cursor-pointer">
            <Filter className="h-4 w-4 mr-1" />
            Filtros
          </div>
        </div>
        <div className="relative">
          <Search className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-neutral-400" />
          <Input
            id="search-opme"
      ...
      <div className="mb-6">
        <h4 className="font-medium text-neutral-700 mb-2">Itens Recomendados</h4>
        ...
        {isLoading ? (
          ...
          recommendedItems.map(item => (
            <div 
              key={item.id}
              className="border border-neutral-200 rounded-lg p-3 mb-3 hover:border-primary-200 hover:bg-primary-50 transition-colors"
            >
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center">
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className="text-primary-500 hover:bg-primary-100 rounded"
        ...
        {searchTerm && !isLoading && (
          <div className="mt-4">
            <h4 className="font-medium text-neutral-700 mb-2">Resultados da Busca</h4>
      ...
      <div>
        <h4 className="font-medium text-neutral-700 mb-2">Itens Selecionados</h4>
        ...
        {selectedItems.length === 0 ? (
          ...
          selectedItems.map(({ item, quantity }) => (
            <div 
              key={item.id}
              className="border border-neutral-200 rounded-lg p-3 mb-3 bg-neutral-50"
            >
              <div className="flex justify-between items-center">
                ...
                <div className="flex-shrink-0 flex items-center">
                  <div className="mr-3">
                    <Label className="text-xs text-neutral-500 block">Quantidade</Label>
                    <Select 
                      value={quantity.toString()} 
                      onValueChange={(val) => updateQuantity(item.id, parseInt(val))}
                    >
                      <SelectTrigger className="border border-neutral-300 rounded w-16 h-8">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {[1, 2, 3, 4, 5].map(num => (
