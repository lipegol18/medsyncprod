      ...
      <Card className="mb-6 bg-[#1a2332] border-blue-800 text-white">
        ...
        <CardContent>
          <div className="space-y-6">
            ...
            <div>
              <Label htmlFor="medicalReport" className="text-white text-lg font-medium">Laudos do Exame</Label>
              ...
              >
                <div 
                  className={cn(
                    "border-2 border-dashed border-blue-700 rounded-lg p-4 mt-1",
                    "transition-all duration-200 hover:border-blue-500",
                    reportPreviews.length > 0 ? "bg-blue-900/20" : "bg-blue-950/30 cursor-pointer"
                  )}
                  onClick={() => {
                    if (medicalReportInputRef.current) {
            ...
            {reportPreviews.length > 0 && (
              <div className="mt-4">
                <p className="text-sm text-blue-300 mb-2">Laudos selecionados ({reportPreviews.length}):</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {reportPreviews.map((report, index) => (
                    <div key={index} className="relative group border border-blue-700 rounded-md p-2 bg-blue-950/30 hover:border-blue-500 transition-colors">
                      <div className="relative">
                        {report.name.toLowerCase().includes('.pdf') ? (
                          <div className="flex flex-col items-center p-4 min-h-[100px] bg-red-900/20 rounded">
                            ...
                            <span className="text-xs text-white text-center break-words">{report.name}</span>
                          </div>
                        ) : (
                          <div className="relative pt-[75%] overflow-hidden">
                            <img 
                        ...
                        {report.name.toLowerCase().includes('.pdf') && (
                          <div className="absolute bottom-1 left-1">
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-6 w-6 rounded-full p-0 opacity-80 hover:opacity-100 bg-blue-600/80"
                              onClick={(e) => {
                                e.stopPropagation();
                                setSelectedImage(report.preview);
                                setImageDialogOpen(true);
