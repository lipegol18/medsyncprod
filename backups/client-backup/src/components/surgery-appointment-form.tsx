...
const surgeryAppointmentFormSchema = z.object({
  ...
  surgeryType: z.enum(['eletiva', 'urgencia', 'emergencia']),
    ...
    <div className="space-y-6">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          {/* Seleção do pedido médico e hospital */}
          {mode === 'create' && (
            <Card>
              <CardHeader>
              ...
              <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Pedido Médico</FormLabel>
                        <Select 
                          onValueChange={(value) => {
                    ...
                    <div className="flex items-center gap-2 p-3 border rounded-md bg-muted/10">
                      {selectedOrder ? (
                        <>
                          ...
                          <span className="font-medium text-sm">{selectedOrder.hospitalName}</span>
                        </>
                      ) : (
                        <span className="text-muted-foreground text-sm">Selecione um pedido médico primeiro</span>
                      )}
                    </div>
                  </div>
                </div>
