...
import { users, roles, medicalOrders, cidCodes, procedures, cidCbhpmAssociations, insertCidCodeSchema, medicalOrderCids, medicalOrderProcedures, medicalOrderOpmeItems, medicalOrderSuppliers, opmeItems, suppliers, surgicalApproaches, insertSurgicalApproachSchema, surgicalApproachProcedures, insertSurgicalApproachProcedureSchema, surgicalApproachOpmeItems, insertSurgicalApproachOpmeItemSchema, surgicalApproachSuppliers, insertSurgicalApproachSupplierSchema, clinicalJustifications, insertClinicalJustificationSchema, surgicalApproachJustifications, insertSurgicalApproachJustificationSchema, medicalOrderSurgicalApproaches, insertMedicalOrderSurgicalApproachSchema, medicalOrderStatusHistory, insertMedicalOrderStatusHistorySchema, orderStatuses, anatomicalRegions, surgicalProcedures, anatomicalRegionProcedures, surgicalProcedureApproaches, insertSurgicalProcedureApproachSchema } from "../shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(`Valores recebidos atualizados com sucesso para ${procedures.length} procedimentos`);
  ...
  app.get("/api/medical-orders/:orderId/opme-items",  async (req: Request, res: Response) => {
    try {
      ...
      // Buscar materiais OPME do pedido com JOIN para obter dados completos
      const result = await db
        .select({
  ...
  app.get("/api/clinical-justifications",  async (req: Request, res: Response) => {
    try {
      ...
      let query = db.select({
        ...
        title: clinicalJustifications.title,
        content: clinicalJustifications.content,
        category: clinicalJustifications.category,
        specialty: clinicalJustifications.specialty,
        procedureType: clinicalJustifications.procedureType,
      ...
      const conditions = [];
      if (category) conditions.push(eq(clinicalJustifications.category, category as string));
      if (specialty) conditions.push(eq(clinicalJustifications.specialty, specialty as string));
      if (procedureType) conditions.push(eq(clinicalJustifications.procedureType, procedureType as string));
      ...
      const justifications = await query.orderBy(clinicalJustifications.category, clinicalJustifications.title);
  ...
  app.get("/api/clinical-justifications/search/:term",  async (req: Request, res: Response) => {
    try {
      ...
      // Filtrar resultados que contenham o termo de busca no título ou conteúdo
      const filteredJustifications = justifications.filter(j => 
        ...
        j.specialty?.toLowerCase().includes(searchTerm)
      );
  ...
  app.post("/api/clinical-justifications",  async (req: Request, res: Response) => {
    try {
      const { title, content, category, specialty, procedureType, isActive } = req.body;
      const userId = req.user?.id;
      ...
      // Verificar se já existe justificativa com mesmo título
      const existingJustification = await db.select()
        .from(clinicalJustifications)
        .where(eq(clinicalJustifications.title, title.trim()));
      
      if (existingJustification.length > 0) {
        return res.status(409).json({ 
          message: "Já existe uma justificativa com este título" 
        });
      }
      ...
      console.log(`Nova justificativa clínica criada: "${title}" (ID: ${newJustification[0].id})`);
  ...
  app.put("/api/clinical-justifications/:id",  async (req: Request, res: Response) => {
    try {
      ...
      // Verificar título duplicado se está sendo alterado
      if (title && title !== existing[0].title) {
        const duplicateTitle = await db.select()
          .from(clinicalJustifications)
          .where(and(
            eq(clinicalJustifications.title, title.trim()),
            // Excluir o próprio registro da verificação
            eq(clinicalJustifications.id, id)
          ));
      ...
      const updatedJustification = await db.update(clinicalJustifications)
        .set({
          ...
          category: category !== undefined ? (category?.trim() || null) : existing[0].category,
          specialty: specialty !== undefined ? (specialty?.trim() || null) : existing[0].specialty,
          procedureType: procedureType !== undefined ? (procedureType?.trim() || null) : existing[0].procedureType,
  ...
  app.get("/api/clinical-justifications/categories",  async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Retornando ${categoryList.length} categorias de justificativas`);
  ...
  app.get("/api/medical-order-surgical-approaches",  async (req: Request, res: Response) => {
    try {
      const associations = await db.select({
        ...
        isPrimary: medicalOrderSurgicalApproaches.isPrimary,
        justificationUsed: medicalOrderSurgicalApproaches.justificationUsed,
        additionalNotes: medicalOrderSurgicalApproaches.additionalNotes,
        surgicalApproachName: surgicalApproaches.name,
        surgicalApproachDescription: surgicalApproaches.description,
  ...
  app.post("/api/medical-order-surgical-approaches",  async (req: Request, res: Response) => {
    try {
      const { medicalOrderId, surgicalApproachId, isPrimary, justificationUsed, additionalNotes } = req.body;
      ...
      if (existingAssociation.length > 0) {
        ...
        const [updatedAssociation] = await db.update(medicalOrderSurgicalApproaches)
          .set({
            ...
            justificationUsed: justificationUsed?.trim() || null,
  ...
  app.put("/api/medical-order-surgical-approaches/:id",  async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const { isPrimary, justificationUsed, additionalNotes } = req.body;
      ...
      const updatedAssociation = await db.update(medicalOrderSurgicalApproaches)
        .set({
          ...
          justificationUsed: justificationUsed !== undefined ? (justificationUsed?.trim() || null) : existing[0].justificationUsed,
          additionalNotes: additionalNotes !== undefined ? (additionalNotes?.trim() || null) : existing[0].additionalNotes,
          updatedAt: new Date()
        })
        .where(eq(medicalOrderSurgicalApproaches.id, id))
        .returning();
  ...
  app.get("/api/surgical-approaches/:id/complete",  async (req: Request, res: Response) => {
    try {
      ...
      const primaryCidId = req.query.primaryCidId ? parseInt(req.query.primaryCidId as string) : null;

      if (isNaN(approachId)) {
        return res.status(400).json({ message: "ID da conduta cirúrgica inválido" });
      }

      console.log(`🔍 Buscando dados completos da conduta cirúrgica ${approachId}${primaryCidId ? ` para CID ${primaryCidId}` : ''}`);
      ...
      // Buscar procedimentos CBHPM associados
      // Se primaryCidId for fornecido, buscar apenas associações específicas para esse CID
      const procedureWhereCondition = primaryCidId 
        ? and(
            eq(surgicalApproachProcedures.surgicalApproachId, approachId),
            eq(surgicalApproachProcedures.cidCodeId, primaryCidId)
          )
        : and(
            eq(surgicalApproachProcedures.surgicalApproachId, approachId),
            // Buscar apenas associações genéricas (sem CID específico) se nenhum CID for fornecido
            isNull(surgicalApproachProcedures.cidCodeId)
          );
      ...
      const associatedProcedures = await db
        ...
        .where(procedureWhereCondition)
        .orderBy(surgicalApproachProcedures.isPreferred);

      // Buscar itens OPME associados
      // Se primaryCidId for fornecido, buscar apenas associações específicas para esse CID
      const opmeWhereCondition = primaryCidId 
        ? and(
            eq(surgicalApproachOpmeItems.surgicalApproachId, approachId),
            eq(surgicalApproachOpmeItems.cidCodeId, primaryCidId)
          )
        : and(
            eq(surgicalApproachOpmeItems.surgicalApproachId, approachId),
            // Buscar apenas associações genéricas (sem CID específico) se nenhum CID for fornecido
            isNull(surgicalApproachOpmeItems.cidCodeId)
          );
      ...
      const associatedOpmeItems = await db
        ...
        .where(opmeWhereCondition)
        .orderBy(surgicalApproachOpmeItems.isRequired);

      // Buscar fornecedores associados
      // Se primaryCidId for fornecido, buscar apenas associações específicas para esse CID
      const supplierWhereCondition = primaryCidId 
        ? and(
            eq(surgicalApproachSuppliers.surgicalApproachId, approachId),
            eq(surgicalApproachSuppliers.cidCodeId, primaryCidId)
          )
        : and(
            eq(surgicalApproachSuppliers.surgicalApproachId, approachId),
            // Buscar apenas associações genéricas (sem CID específico) se nenhum CID for fornecido
            isNull(surgicalApproachSuppliers.cidCodeId)
          );
      ...
      // Buscar justificativas clínicas associadas
      // Se primaryCidId for fornecido, buscar apenas associações específicas para esse CID
      const justificationWhereCondition = primaryCidId 
        ? and(
            eq(surgicalApproachJustifications.surgicalApproachId, approachId),
            eq(surgicalApproachJustifications.cidCodeId, primaryCidId)
          )
        : and(
            eq(surgicalApproachJustifications.surgicalApproachId, approachId),
            // Buscar apenas associações genéricas (sem CID específico) se nenhum CID for fornecido
            isNull(surgicalApproachJustifications.cidCodeId)
          );
      ...
      const justifications = await db
        ...
        .where(justificationWhereCondition)
        .orderBy(surgicalApproachJustifications.isPreferred);
