...
export const surgicalApproachOpmeItems = pgTable("surgical_approach_opme_items", {
  ...
  notes: text("notes"), // Observações específicas para esta associação
  cidCodeId: integer("cid_code_id").references(() => cidCodes.id, { onDelete: 'cascade' }), // CID específico para esta associação
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
// Tabela de associação entre condutas cirúrgicas e fornecedores (N:M com limite de 3 fornecedores)
export const surgicalApproachSuppliers = pgTable("surgical_approach_suppliers", {
  id: serial("id").primaryKey(),
  surgicalApproachId: integer("surgical_approach_id").notNull().references(() => surgicalApproaches.id, { onDelete: 'cascade' }),
  supplierId: integer("supplier_id").notNull().references(() => suppliers.id, { onDelete: 'cascade' }),
  priority: integer("priority").default(1), // Prioridade do fornecedor (1-3)
...
export const medicalOrderSurgicalApproaches = pgTable("medical_order_surgical_approaches", {
  ...
  justificationUsed: text("justification_used"), // Texto da justificativa utilizada para esta conduta
  additionalNotes: text("additional_notes"), // Observações específicas para esta conduta no pedido
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const orderStatusEnum = pgEnum("order_status", ["em_preenchimento", "em_avaliacao", "aceito", "autorizado_parcial", "pendencia", "cirurgia_realizada", "cancelado"]);
...
// Medical Order OPME Items - Relacionamento N:N entre pedidos e itens OPME
