...
interface SurgicalCalendarBigProps {
  ...
  onUpdateAppointment: (
    ...
    updates: Partial<SurgeryAppointment>,
  ) => void;
}
...
export function SurgicalCalendarBig({
  ...
  // Garantir locale pt-br sempre ativo
  moment.locale("pt-br");
  ...
  const events: CalendarEvent[] = useMemo(() => {
    ...
    const processedEvents = uniqueAppointments
      .map((appointment) => {
        ...
        const [hours, minutes] = appointment.scheduledTime
          .split(":")
          .map(Number);
  ...
  const handleEventDrop = async (args: any) => {
    ...
    console.log("🎯 Movendo agendamento:", {
      appointmentId,
      start,
      end,
      newTime,
      ...
      <div className="container mx-auto p-6 space-y-6">
        <div className="h-full flex flex-col">
          ...
          <div className="flex items-center justify-between mb-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
            <button onClick={handlePrevMonth} className="btn-medsync-dark">
              Anterior
            </button>
            <h3 className="text-lg font-semibold text-sky-800">
              {moment(currentDate).locale("pt-br").format("MMMM [de] YYYY")}
            </h3>
            <div className="flex gap-2">
              <button
                onClick={handleToday}
                className="btn-medsync-dark"
              >
                Hoje
              </button>
              <button
          ...
          <div className="flex-1 bg-white rounded-lg border border-gray-200 shadow-sm overflow-auto">
            <div className="grid grid-cols-7 h-full">
              ...
              {weekDays.map((day) => (
                <div
                  key={day}
                  className="border-b border-r border-gray-200 text-center font-semibold"
                  style={{ 
                    backgroundColor: 'hsl(var(--accent-light))',
                    color: 'hsl(var(--medsync-dark-blue))',
                    padding: '12px 8px'
                  }}
                >
                  {day}
              ...
              {monthDays.map((day, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`border-b border-r border-gray-200 p-2 min-h-[100px] ${isToday ? "bg-sky-50" : ""} ${!day ? "bg-gray-50" : ""}`}
                  >
                    {day && (
                      <>
                        <div
                          className={`text-sm font-medium mb-1 ${isToday ? "text-sky-700 font-bold" : "text-gray-700"}`}
                        >
                          {day.getDate()}
                        </div>
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="h-full flex flex-col">
        ...
        {/* Navegação do dia - aparece apenas na visualização Dia */}
        {currentView === Views.DAY && (
          <div className="flex items-center justify-between mb-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
            ...
            <h3 className="text-lg font-semibold text-sky-800">
              {moment(currentDate).locale("pt-br").format("dddd, DD/MM/YYYY")}
            </h3>
            <div className="flex gap-2">
              <button
        ...
        {/* Navegação da semana - aparece apenas na visualização Semana */}
          ...
          <DragAndDropCalendar
            ...
            formats={{
              ...
              dayHeaderFormat: (date: Date) => {
                ...
                return `${dayNames[moment(date).day()]}, ${moment(date).locale("pt-br").format("DD [de] MMMM")}`;
              },
